...
test('registry: exposes all ten tools with coherent schemas', () => {
  const { tools, cleanup } = setup();
  try {
    assert.deepEqual(Object.keys(tools).sort(), ['bash', 'dedup', 'edit', 'glob', 'grep', 'junkscan', 'ls', 'read', 'skill', 'write']);
